...
<div class="social-vote">
    <div class="social-comment <?= $commStand ?>">
        <div class="social-comment-ibox" name="comment-<?= $comm['commentid'] ?>" data-rows="<?= $replyRows ?>">
            ...
            <?php if ($comm['replies']) {
                foreach ($comm['replies'] as $key => $reply) {
                    switch ($reply['stand']) {
                        case 1:
                ...
                <div class="reply <?= $standClass ?>" id="reply-<?= $reply['replyid'] ?>" data-node="reply">
                    ...
                    <div class="comment-body">
                        ...
                        <div class="comment-word">
                            <div name="content-md" id="md-<?= $comm['commentid'].'-'.$reply['replyid'] ?>"><?= Html::encode($reply['content']); ?></div>
                        </div>
                        <div class="comment-footer"><?= $standStr ?>
                            <?php if (Yii::$app->user->getId() != $reply['user']['uid']) { ?>
                                <a href="javascript:;" class="small" name="reply-btn" data-iscomment="0" reply-author="<?= $reply['user']['nickname'] ?>" reply-to="<?= $reply['user']['uid'] ?>" reply-comment= "<?= $comm['commentid'] ?>">
                                    回复<i class="fa fa-mail-reply"></i>
                                </a>
                            <?php }?>
                        </div>
            ...
            <?php }
                if ($replyRows >= Yii::$app->params['replyPageSize']) {?>
                <div class="reply-more">
                    <button type="button" class="btn btn-sm" name="js-reply-more" data-page="1" data-comment="<?= $comm['commentid'] ?>">加载更多…</button>
            <?php }}?>
        </div>
    </div>
</div>
